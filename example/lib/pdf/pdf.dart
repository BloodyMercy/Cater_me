...
class _MyPdfState extends State<MyPdf> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
            title:Text(
                '${authProvider.lg[authProvider.language]["PDF Invoice"]}'
                ), //appbar title
            // actions:[ IconButton(icon: Icon(Icons.download_outlined,color: Colors.white,),
            // onPressed: (){},),],
            ...
            child: PDF().cachedFromUrl(
             ...
             orders.pdfLink,
              ...
              placeholder: (progress) => Center(child: Text('$progress %')),
               errorWidget: (error) { Timer(Duration(seconds: 1,milliseconds: 500), (){Navigator.pop(context);});  return  Center(child: Text('${authProvider.lg[authProvider.language]["failed ,try again later..."]}'));},
             )
        )
    );
